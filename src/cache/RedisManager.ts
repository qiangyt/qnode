 ...
 */
export default class RedisManager extends Bean {
    ...
    create( instanceName:string ) {
        let instanceConfig;
        if( !instanceName ) {
            instanceConfig = this._config;
        } else {
            if( !this._config[instanceName] ) throw new Error('please configure section: redis.' + instanceName);

            instanceConfig = _.cloneDeep(this._config);
            _.merge( instanceConfig, this._config[instanceName] );
            this._config[instanceName] = instanceConfig;
        }
